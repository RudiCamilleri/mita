...
interface TenderBLLInterface {
	...
	//Creates an order
	function createOrder(uint32 contractId, uint32 orderId, uint16 small, uint16 medium, uint16 large) external;

	//to confirm order is delivered
	function markDelivered(uint32 contractId, uint32 orderId) external; //uint32 - Order number
